    ...
    input.addEventListener('keydown', (e) => { if (e.key === 'Enter') sendMessage(); });
      ...
      registerApiRoute("/chat", {
        ...
        handler: async (c) => {
          ...
          let reply: string;
          try {
            // Use generate method which is the standard Mastra agent API
            const response = await (agent as { generate: (input: string) => Promise<{ text: string }> }).generate(message);
            reply = response.text;
          } catch (e) {
...
try {
  ...
  console.warn("Telegram trigger registration skipped:", e instanceof Error ? e.message : e);
}
