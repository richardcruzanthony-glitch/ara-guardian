...
2. **System Topology**  
  - `src/mastra/index.ts`: boots Mastra, registers tools, exposes MCP + Telegram trigger, hosts HTTP server.  
  ...
  - Triggers under `src/triggers/` connect Slack, Telegram, cron, daily scrape; `inngest/` adds background workflows.  
...
5. **Integration Contracts**  
  - LLM credentials: prefer Replit AI (`AI_INTEGRATIONS_OPENAI_BASE_URL` / `_API_KEY`), otherwise fall back to standard `OPENAI_API_KEY` + optional `OPENAI_BASE_URL`; Grok/xAI tooling (`grokReasoning`) requires `GROK_API_KEY`.  
  - Telegram trigger expects `BOT_TOKEN`; Slack trigger requires `SLACK_BOT_TOKEN`.  
...
6. **Common Pitfalls**  
  ...
  - `node-telegram-bot-api` is not installed by default; either add it or stick to the webhook trigger.  
